...
class TokenUsageService {
  ...
  async recordTokenUsage(record: Omit<TokenUsageRecord, 'id' | 'timestamp'>): Promise<void> {
    ...
    const userPart = record.userId.substr(-8); // 使用用户ID的后8位
    const uniqueId = `token_${timestamp}_${userPart}_${randomPart}`;
  ...
  private async saveTokenUsageToDatabase(record: TokenUsageRecord): Promise<void> {
    try {
      ...
      // 🔧 修夏：转换字段名以匹配数据库schema
      const dbRecord = {
        id: record.id,
        user_id: record.userId,  // camelCase -> snake_case
        feature: record.feature,
      ...
      // 使用直接的Supabase客户端进行插入，避免字段名转换问题
      ...
      if (error) {
        logger.error('❌ 插入Token记录失败:', error);
      ...
      logger.error('❌ 保存Token使用记录到Supabase失败:', error);
      // 🚨 数据库失败时必须抛出错误，不能使用localStorage回退
      throw new Error(`Supabase数据库操作失败: ${error instanceof Error ? error.message : i18n.t('common.errors.未知错误')}`);
    }
  }
