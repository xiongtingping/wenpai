...
if (!openaiKey) {
  ...
  console.log('❌ 长度过短：至少需要20个字符');
} else {
  console.log('✅ 格式正确');
  console.log(`   - 长度: ${openaiKey.length}字符`);
  console.log(`   - 前缀: ${openaiKey.substring(0, 10)}...`);
}
...
if (!deepseekKey) {
  ...
  console.log('✅ 格式正确');
  console.log(`   - 长度: ${deepseekKey.length}字符`);
  console.log(`   - 前缀: ${deepseekKey.substring(0, 10)}...`);
}
...
if (!geminiKey || geminiKey.includes('your-')) {
  ...
  console.log('✅ 已配置');
  console.log(`   - 长度: ${geminiKey.length}字符`);
}
