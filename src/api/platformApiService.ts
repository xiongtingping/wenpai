...
async function publishToWeibo(content: PublishContent, config: PlatformApiConfig): Promise<PublishResult> {
  try {
    ...
    if (data.id) {
      ...
      return { success: false, error: data.error || 'u64cdu4f5cu5931u8d25' };
    }
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : 'u64cdu4f5cu5931u8d25' };
  }
}
...
async function publishToZhihu(content: PublishContent, config: PlatformApiConfig): Promise<PublishResult> {
  try {
    ...
    const draftData = await request.post(`${config.apiBaseUrl}/drafts`, {
      title: content.title || 'u64cdu4f5cu5931u8d25',
...
async function publishToTwitter(content: PublishContent, config: PlatformApiConfig): Promise<PublishResult> {
  try {
    ...
    if (data.data?.id) {
      ...
      return { success: false, error: data.errors?.[0]?.message || 'u64cdu4f5cu5931u8d25' };
    }
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : 'u64cdu4f5cu5931u8d25' };
  }
