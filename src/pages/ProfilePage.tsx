    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      <div className="container mx-auto px-4 pt-6 pb-8 max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* 个人资料与联系方式合并卡片 */}
          <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm md:col-span-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
          {currentPlan && (<UsageStatsCard usageStats={usageStats} planName={currentPlan.name} adaptUsageLimit={currentPlan.limits.adaptUsageLimit} tokenLimit={currentPlan.limits.tokenLimit} userType={userType} onUpgrade={() => navigate('/payment')} />)}
          <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
            <CardHeader className="pb-6 bg-gradient-to-r from-orange-500 to-red-600 text-white rounded-t-lg">
              <CardTitle className="flex items-center justify-between text-xl"><div className="flex items-center gap-3"><div className="p-2 bg-white/20 rounded-lg"><Gift className="w-6 h-6" /></div>奖励机制</div><Button variant="outline" onClick={() => navigate('/invite')} size="sm" className="bg-white/20 border-white/30 text-white hover:bg-white/30"><User className="w-4 h-4 mr-2" />立即邀请</Button></CardTitle>
            </CardHeader>
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg border border-blue-200"><div className="flex items-center gap-3"><div className="p-2 bg-blue-500 rounded-lg"><User className="w-4 h-4 text-white" /></div><span className="text-gray-700">每邀请1人注册</span></div><Badge className="bg-blue-500 text-white">双方各获得20次</Badge></div>
                    <div className="flex items-center justify-between p-4 bg-gradient-to-r from-green-50 to-green-100 rounded-lg border border-green-200"><div className="flex items-center gap-3"><div className="p-2 bg-green-500 rounded-lg"><Gift className="w-4 h-4 text-white" /></div><span className="text-gray-700">使用次数永久有效</span></div><Badge className="bg-green-500 text-white">不会过期</Badge></div>
                  </div>
                </div>
                <div className="space-y-4">
                  <h5 className="font-semibold text-gray-800 mb-3">邀请统计</h5>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gradient-to-r from-purple-50 to-purple-100 p-4 rounded-lg border border-purple-200 text-center"><div className="text-2xl font-bold text-purple-600 mb-1">{userInviteStats.totalRegistrations}</div><div className="text-sm text-gray-600">成功邀请</div></div>
                    <div className="bg-gradient-to-r from-green-50 to-green-100 p-4 rounded-lg border border-green-200 text-center"><div className="text-2xl font-bold text-green-600 mb-1">{userInviteStats.totalRegistrations * 20}</div><div className="text-sm text-gray-600">获得次数</div></div>
                  </div>
                  <Button onClick={() => navigate('/invite')} className="w-full bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white"><User className="w-4 h-4 mr-2" />邀请好友获得奖励</Button>
                </div>
              </div>
            </CardContent>
          </Card>
